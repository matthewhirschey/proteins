...
load_normalize <- function(proteins_raw, peptides_raw, num_input, num_group, num_exp, num_pool){
  ...
  for (i in 1:num_input) {
    user_input <-
      dlg_input("Enter a string that is specific to and common among the input column names in the raw PEPTIDE data.")$res
  ...
  for(i in 1:num_group){
    user_input <-
      dlg_input("Enter a string that is specific to and common among all column names for your experimental group PEPTIDE data.")$res
    group_string_ids[[i]] <- user_input
    index_list[[i]] <- grep(user_input, names(peptides_raw))
  }
  peptides_load_normalized <- bind_cols(tibble(unique_id = 1:nrow(peptides_raw)),
                                        select(peptides_raw, "Master Protein Accessions"),
                                        select(peptides_raw, "Sequence"))
  peptides_load_normalized$Accession <- str_split_fixed(peptides_load_normalized$`Master Protein Accessions`, ";", 2)[,1]
  peptides_load_normalized <- left_join(peptides_load_normalized, select(proteins_raw, Accession, Description), by = "Accession")
  peptides_load_normalized$GN <- str_extract(peptides_load_normalized$Description, "(?<=GN=).*?(?=\\s|$)")
  ...
  k = 1
  for(i in 1:num_group){
    df <- bind_cols(unique_id = 1:nrow(peptides_raw),
                    select(peptides_raw, index_list[[i]]))
    ...
    peptides_load_normalized_list[[i]] <- left_join(peptides_load_normalized, df, by = "unique_id")
  }

  # Initialize the proteins_load_normalized data structure
  proteins_load_normalized <- bind_cols(tibble(unique_id = 1:nrow(proteins_raw)),
  ...
  for (i in 1:num_input){
    ...
    df <- bind_cols(unique_id = 1:nrow(proteins_raw),
    ...
    proteins_load_normalized_list[[i]] <- left_join(proteins_load_normalized, df, by = "unique_id")
  }
